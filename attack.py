...
原理：检测器在 8×8 分块的中频 DCT 系数上计算归一化相关系数。相关系数只取决于
系数中沿伪随机序列的分量，因此"削弱水印强度"没有意义（归一化对整体缩放不敏感），
必须把该分量擦除。做法是对中高频系数做粗量化：当量化步长大于水印引起的偏移时，
系数会被映射回原本的量化格点，水印分量在统计意义上被抹平，相关系数随之落到噪声带。

与旧方案的两点区别：
  1) 不再使用"先猛攻、再朝原图回拉"的预算控制——回拉会把原图的块间对齐关系带回来，
     反而恢复水印的可检测性；
  2) 不再把质量预算花在全图几何形变上——其代价随分辨率与内容复杂度爆炸，
     而系数域量化的代价与内容解耦，分辨率越高越容易满足预算。

量化步长由质量预算在系数域解析反解（正交变换下系数域误差能量等于像素域误差能量），
取满足 PSNR 门槛的最大步长，从而使擦除能力最大化。可选叠加一次很小的几何扰动，
用于打散量化后残留的块间对应关系。

依赖：仅 numpy。不使用随机数、不访问检测器、不联网，相同输入恒得相同输出。
"""
